    ...
    def _write_tfrecord_file(self, shard_data):
        ...
        with tf.io.TFRecordWriter(shard_path, options='ZLIB') as out:
            for index in indices:
                ...
                rate = 16000
                stream = tfio.audio.AudioIOTensor(file_path)
                audio = tfio.audio.resample(stream.to_tensor(), tf.cast(stream.rate, tf.int64), rate, name='Resampler')
                a_shape = audio.shape
                w_len = int(rate * self.duration)
                start = random.randint(0, a_shape[0] - w_len)
                stop = start + w_len
                audio_tensor = tf.reduce_mean(audio, axis=-1)  # [samples, channels] => [samples]
                # audio_tensor = tf.squeeze(audio[start:stop, ][:, 1])
                zeros = tf.math.maximum(w_len - tf.shape(audio_tensor)[0], 0)
                paddings = [[zeros // 2, zeros // 2 + zeros % 2]]
                pad_audio = tf.pad(audio_tensor, paddings=paddings, mode='CONSTANT')  # pad if audio is too short
                for nth_example in range(80):
                    cropped_audio = tf.image.random_crop(pad_audio, [w_len])

                    tensor = tf.cast(cropped_audio, tf.float32) / tf.reduce_max(tf.abs(cropped_audio))
                    spectrogram = tfio.experimental.audio.spectrogram(tensor, nfft=512, window=512, stride=256)
                    mel = tfio.experimental.audio.melscale(spectrogram, rate=rate, mels=128, fmin=100,
                                                           fmax=rate / 2)
                    db = tfio.experimental.audio.dbscale(mel, top_db=80)
                    db = tf.expand_dims(db, -1) #tf.reshape(db, [313, 128, 1])
                    db = ((db + 80) / 80.0) * 255
                    db = tf.cast(db, dtype=tf.uint8)
                    db_img = tf.io.encode_png(db, name='png_encoder')
                    ...
                    example = tf.train.Example(features=tf.train.Features(feature={
                        'mel': _bytes_feature(db_img),
                        'bands': _int64_feature(db.shape[0]),
                        'bins': _int64_feature(db.shape[1]),
                        'sr': _int64_feature(rate),
                        'label': _int64_feature(label)}))
                    # print(db.numpy().shape)
